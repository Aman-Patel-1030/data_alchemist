...
const fixInvalidTaskReferences = () => {
  const updatedClients = state.clients.map((client) => {
    let validTaskIds = client.RequestedTaskIDs;

    // Step 1: Fix invalid RequestedTaskIDs
    if (Array.isArray(client.RequestedTaskIDs)) {
      validTaskIds = client.RequestedTaskIDs.filter((taskId) => {
        return (
          !taskId.includes("X") &&
          !taskId.includes("99") &&
          !taskId.includes("60") &&
          !taskId.includes("51") &&
          /^T\d+$/.test(taskId) &&
          parseInt(taskId.substring(1)) <= 50
        );
      });
    }

    // Step 2: Fix malformed AttributesJSON
    let fixedAttributesJSON = client.AttributesJSON;

    if (fixedAttributesJSON && typeof fixedAttributesJSON === "string") {
      const trimmed = fixedAttributesJSON.trim();

      if (trimmed.startsWith("{")) {
        try {
          const parsed = JSON.parse(trimmed);
          fixedAttributesJSON = JSON.stringify(parsed);
        } catch (e) {
          const fixedStr = trimmed
            .replace(/([{,]\s*)([a-zA-Z0-9_]+)\s*:/g, '$1"$2":') // wrap keys
            .replace(/:\s*([^,"{}\n]+)/g, ': "$1"'); // wrap unquoted values

          try {
            const parsed = JSON.parse(fixedStr);
            fixedAttributesJSON = JSON.stringify(parsed);
          } catch (err2) {
            fixedAttributesJSON = JSON.stringify({ message: trimmed });
          }
        }
      } else {
        fixedAttributesJSON = JSON.stringify({ message: trimmed });
    ...
    return {
      ...client,
      RequestedTaskIDs: validTaskIds,
      AttributesJSON: fixedAttributesJSON,
    };
  });
  ...
  const updatedClients = state.clients.map((client) => {
    const raw = client.AttributesJSON;
    ...
    if (raw && typeof raw === "string") {
      ...
      if (trimmed.startsWith("{")) {
        try {
          ...
          // Try to fix the JSON string using regex
          const fixedStr = trimmed
            .replace(/([{,]\s*)([a-zA-Z0-9_]+)\s*:/g, '$1"$2":') // wrap keys
            .replace(/:\s*([^,"{}👦👦]+)/g, ': "$1"'); // wrap unquoted values
          ...
          try {
            ...
            // Still broken — last resort fallback
            return {
              ...client,
              AttributesJSON: JSON.stringify({ message: trimmed }),
            };
          }
        }
      } else {
        // Not a JSON string at all — wrap as message
        return {
          ...client,
          AttributesJSON: JSON.stringify({ message: trimmed }),
        };
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2">
          <Sparkles className="h-5 w-5 bg-gradient-to-br from-primary to-black text-white" />
          <CardTitle className="text-lg">AI Error Correction</CardTitle>
        </div>
        <CardDescription>AI-powered suggestions to fix data quality issues and optimize your dataset</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {/* Analysis Button */}
        <Button onClick={generateSuggestions} disabled={isAnalyzing} className="w-full bg-gradient-to-br from-primary to-black text-white">
          {isAnalyzing ? (
            <>
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {!isApplied && suggestion.autoApplicable && (
                          <Button 
                          className="bg-gradient-to-br from-primary to-black text-white hover:from-primary/90 hover:0"
                            size="sm"
                            variant="outline"
